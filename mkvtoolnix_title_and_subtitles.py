...
class MKVProcessor:
    ...
    def remove_title_keep_english_subs(self, file_path, output_path):
        ...
        try:
            ...
            logging.error("Error processing %s: %s", file_path.name, e)
            self._write_or_append_to_json(f"Error processing {file_path.name}: {e}", is_success=False)
            return False
...
if __name__ == "__main__":
    CONFIG_PATH = '/path/to/config/json/file'  # Path to your configuration file
