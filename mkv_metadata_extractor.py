...
Usage:
    python mkv_metadata_extractor.py <root_directory>
...
Examples:
    ...
    python mkv_metadata_extractor.py /home/user/videos

    # Show help message:
    python mkv_metadata_extractor.py --help
...
class MKVMetadataExtractor:
    ...
    def _extract_file_metadata(self, file_path: Path) -> FileMetadata:
        ...
        if 'chapters' in metadata:
            processed_metadata["chapters"] = self._extract_chapters_info(metadata['chapters'])

        return processed_metadata

    def _extract_track_info(self, track: Dict[str, Any]) -> TrackInfo:
        """
        ...
        if track['type'] == 'video':
            properties = {
                ...
                "default_track": properties.get('default_track', False),
                "display_dimensions": properties.get('display_dimensions', ''),
                "enabled_track": properties.get('enabled_track', False),
                ...
                "tag_duration": properties.get('tag_duration', '')
            }
        elif track['type'] == 'audio':
            properties = {
                "audio_channels": properties.get('audio_channels', 0),
                "audio_sampling_frequency": properties.get('audio_sampling_frequency', 0),
                "codec_id": properties.get('codec_id', ''),
                "default_track": properties.get('default_track', False),
                "enabled_track": properties.get('enabled_track', False),
                "forced_track": properties.get('forced_track', False),
            ...
            properties = {
                ...
                "number": properties.get('number', 0),
                "tag_duration": properties.get('tag_duration', ''),
                "text_subtitles": properties.get('text_subtitles', False),
                "track_name": properties.get('track_name', '')
            }

        common_info["Properties"] = properties
        return TrackInfo(**common_info)

    def _extract_chapters_info(self, chapters: List[Dict[str, Any]]) -> List[ChapterInfo]:
        """
